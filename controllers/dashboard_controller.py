...
class DashboardController(http.Controller):
    ...
    @http.route('/dashboard_projet/export_complete', type='http', auth='user', methods=['GET'], csrf=False)
    def export_complete_dashboard(self, date_debut=None, date_fin=None, **kwargs):
        """Endpoint pour l'export complet avec graphiques"""
        try:
            _logger.info(f"Export complet dashboard demandé - dates: {date_debut} - {date_fin}")
            ...
            pdf_buffer = self._generate_complete_pdf(data, date_debut, date_fin)
            ...
            _logger.error(f"Erreur export complet dashboard: {str(e)}")
            return request.make_response(f"Erreur export: {str(e)}", status=500)

    def _generate_complete_pdf(self, data, date_debut, date_fin):
        """Génère un PDF complet avec données et représentations graphiques"""
        try:
            buffer = io.BytesIO()
            doc = SimpleDocTemplate(
                ...
                topMargin=1.5 * cm,
                bottomMargin=1.5 * cm,
                title=f"Dashboard Complet {date_debut} - {date_fin}",
            )
            ...
            story.append(Paragraph(f"<b>TABLEAU DE BORD COMPLET</b>", title_style))
            story.append(Paragraph(f"Du {date_debut} au {date_fin}", styles['Heading2']))
            ...
            metrics_data = [
                ['Chiffre d\'affaires total:', f"{data.get('chiffre_affaires', 0):,.0f} CFA"],
                ['Nombre de projets:', str(len(data.get('projets', [])))],
                ['Personnel total:', str(sum(p.get('nb_personnes', 0) for p in data.get('projets', [])))],
                ['Heures totales:', f"{sum(p.get('heures', 0) for p in data.get('projets', [])):,.0f} h"],
                ['Marge administrative:', f"{data.get('marge_administrative', {}).get('taux_marge_admin', 0):.1f}%"]
            ]
            ...
            # Représentations tabulaires des graphiques
            graphique_data = data.get('graphique_data', {})
            
            # CA par projet
            if graphique_data.get('graphique_ca'):
                story.append(Paragraph("<b>CHIFFRE D'AFFAIRES PAR PROJET</b>", styles['Heading2']))
                ca_data = graphique_data['graphique_ca']
                
                if ca_data.get('labels') and ca_data.get('data'):
                    table_data = [['Projet', 'CA (CFA)']]
                    for label, value in zip(ca_data['labels'], ca_data['data']):
                        table_data.append([label, f"{value:,.0f}"])
            ...
            if graphique_data.get('graphique_statuts'):
                story.append(Paragraph("<b>RÉPARTITION PAR STATUT</b>", styles['Heading2']))
                statuts_data = graphique_data['graphique_statuts']
                
                if statuts_data.get('labels') and statuts_data.get('data'):
                    table_data = [['Statut', 'Nombre', '%']]
                    total = sum(statuts_data['data'])
                    
                    for label, value in zip(statuts_data['labels'], statuts_data['data']):
                        percentage = (value / total * 100) if total > 0 else 0
                        table_data.append([label, str(value), f"{percentage:.1f}%"])
            ...
            if data.get('projets'):
                ...
                table_data = [[
                    'Projet', 'CA (CFA)', 'Personnes', 'Heures', 'Statut', 'Marge (%)'
                ]]
                
                for projet in data['projets'][:30]:  # Limiter à 30 projets
                    marge_data = projet.get('marge_data', {}) or {}
                    table_data.append([
                        projet.get('name', '')[:25],
                        f"{projet.get('ca', 0):,.0f}",
                        str(projet.get('nb_personnes', 0)),
                        f"{projet.get('heures', 0):.0f}",
                        projet.get('stage', '')[:15],
                        f"{marge_data.get('taux_marge', 0):.1f}%"
                    ])
            ...
            _logger.error(f"Erreur génération PDF complet: {str(e)}")
            raise e
    ...
    # À ajouter dans dashboard_controller.py
    @http.route('/dashboard_projet/export_graphiques', type='http', auth='user', methods=['POST'], csrf=False)
    def export_graphiques_pdf(self, date_debut=None, date_fin=None, graphiques_data=None, **kwargs):
        """Endpoint pour l'export des graphiques en PDF via le backend"""
        try:
            _logger.info(f"Export graphiques PDF demandé - dates: {date_debut} - {date_fin}")
            ...
            graphique_data = dashboard_model.get_graphique_data(date_debut, date_fin)
            
            # Générer le PDF avec les données des graphiques
            pdf_buffer = self._generate_graphiques_pdf_backend(graphique_data, date_debut, date_fin)
            
            filename = f"graphiques_dashboard_{date_debut}_{date_fin}.pdf"
            return request.make_response(
                pdf_buffer.getvalue(),
                headers=[
                    ('Content-Type', 'application/pdf'),
            ...
            _logger.error(f"Erreur export graphiques PDF: {str(e)}")
            return request.make_response(f"Erreur export: {str(e)}", status=500)

    def _generate_graphiques_pdf_backend(self, graphique_data, date_debut, date_fin):
        """Génère un PDF avec les données des graphiques"""
        try:
            buffer = io.BytesIO()
            doc = SimpleDocTemplate(
                buffer,
                ...
                topMargin=1.5 * cm,
                bottomMargin=1.5 * cm,
                title=f"Graphiques Dashboard {date_debut} - {date_fin}",
            )
            ...
            title = Paragraph(f"<b>GRAPHIQUES DASHBOARD</b><br/>Du {date_debut} au {date_fin}", title_style)
            story.append(title)
            story.append(Spacer(1, 1 * cm))
            
            # Section CA par projet
            if 'graphique_ca' in graphique_data:
                story.append(Paragraph("<b>CHIFFRE D'AFFAIRES PAR PROJET</b>", styles['Heading2']))
                ca_data = graphique_data['graphique_ca']
                
                if ca_data.get('labels') and ca_data.get('data'):
                    table_data = [['Projet', 'CA (CFA)']]
                    for i, (label, value) in enumerate(zip(ca_data['labels'], ca_data['data'])):
                        table_data.append([label, f"{value:,.0f}"])
            ...
            if 'graphique_statuts' in graphique_data:
                story.append(Paragraph("<b>RÉPARTITION PAR STATUT</b>", styles['Heading2']))
                statuts_data = graphique_data['graphique_statuts']
                
                if statuts_data.get('labels') and statuts_data.get('data'):
                    table_data = [['Statut', 'Nombre de projets', 'Pourcentage']]
                    total_projets = sum(statuts_data['data'])
                    
                    for i, (label, value) in enumerate(zip(statuts_data['labels'], statuts_data['data'])):
                        pourcentage = (value / total_projets * 100) if total_projets > 0 else 0
                        table_data.append([label, str(value), f"{pourcentage:.1f}%"])
            ...
            if 'graphique_evolution' in graphique_data:
                story.append(Paragraph("<b>ÉVOLUTION MENSUELLE DU CA</b>", styles['Heading2']))
                evolution_data = graphique_data['graphique_evolution']
                
                if evolution_data.get('labels') and evolution_data.get('data'):
                    table_data = [['Période', 'CA (CFA)', 'Évolution']]
                    prev_value = None
                    
                    for i, (label, value) in enumerate(zip(evolution_data['labels'], evolution_data['data'])):
                        evolution_text = ""
                        if prev_value is not None:
                            diff = value - prev_value
                            if diff > 0:
                                evolution_text = f"+{diff:,.0f} (↗)"
                            elif diff < 0:
                                evolution_text = f"{diff:,.0f} (↘)"
                            else:
                                evolution_text = "= (→)"
                        ...
                        table_data.append([label, f"{value:,.0f}", evolution_text])
                        prev_value = value
            ...
            # Résumé statistique
            story.append(Paragraph("<b>RÉSUMÉ STATISTIQUE</b>", styles['Heading2']))
            
            # Calculer quelques statistiques
            stats_data = []
            
            # CA total si disponible
            if 'graphique_ca' in graphique_data and graphique_data['graphique_ca'].get('data'):
                ca_total = sum(graphique_data['graphique_ca']['data'])
                stats_data.append(['CA Total', f"{ca_total:,.0f} CFA"])
                
                # CA moyen par projet
                nb_projets_ca = len(graphique_data['graphique_ca']['data'])
                if nb_projets_ca > 0:
                    ca_moyen = ca_total / nb_projets_ca
                    stats_data.append(['CA Moyen par Projet', f"{ca_moyen:,.0f} CFA"])
            
            # Nombre total de projets si disponible
            if 'graphique_statuts' in graphique_data and graphique_data['graphique_statuts'].get('data'):
                total_projets = sum(graphique_data['graphique_statuts']['data'])
                stats_data.append(['Nombre Total de Projets', str(total_projets)])
            
            # Évolution globale si disponible
            if 'graphique_evolution' in graphique_data and graphique_data['graphique_evolution'].get('data'):
                evolution_data = graphique_data['graphique_evolution']['data']
                if len(evolution_data) >= 2:
                    evolution_globale = evolution_data[-1] - evolution_data[0]
                    evolution_pct = (evolution_globale / evolution_data[0] * 100) if evolution_data[0] != 0 else 0
                    stats_data.append(['Évolution Globale', f"{evolution_globale:,.0f} CFA ({evolution_pct:+.1f}%)"])
            
            if stats_data:
                stats_table = Table([['Métrique', 'Valeur']] + stats_data, repeatRows=1)
            ...
            # Pied de page
            footer_text = f"Rapport généré le {datetime.now().strftime('%d/%m/%Y à %H:%M')} par {request.env.user.name}"
            footer_para = Paragraph(footer_text, styles['Italic'])
            footer_para.alignment = 1  # Centré
            story.append(footer_para)
            
            # Note technique
            note_text = "<i>Note: Ce rapport contient les données des graphiques sous forme tabulaire. Pour une visualisation graphique complète, utilisez l'export frontend.</i>"
            story.append(Spacer(1, 0.5 * cm))
            ...
            _logger.error(f"Erreur génération PDF graphiques: {str(e)}")
            raise e